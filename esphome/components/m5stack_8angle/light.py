...
M5Stack8AngleLight = m5stack_8angle_ns.class_(
    "M5Stack8AngleLight", light.LightOutput
)

CONFIG_SCHEMA = light.RGB_LIGHT_SCHEMA.extend({
    cv.GenerateID(CONF_OUTPUT_ID): cv.declare_id(M5Stack8AngleLight),
    cv.Required(CONF_M5STACK_8ANGLE_ID): cv.use_id(M5Stack8Angle),
    cv.Required(CONF_NUMBER): cv.int_range(min=0, max=8),
})


async def to_code(config):
    parent = await cg.get_variable(config[CONF_M5STACK_8ANGLE_ID])
