...
def _validate_unique(config):
    encoders = [item[CONF_NUMBER] for item in config[CONF_ENCODERS]]
    buttons = [item[CONF_NUMBER] for item in config[CONF_BUTTONS]]
    if len(encoders) != len(set(encoders)):
        raise cv.Invalid("Każdy numer enkodera może wystąpić tylko raz")
    if len(buttons) != len(set(buttons)):
        raise cv.Invalid("Każdy numer przycisku może wystąpić tylko raz")
    return config

CONFIG_SCHEMA = cv.All(CONFIG_SCHEMA, _validate_unique)
