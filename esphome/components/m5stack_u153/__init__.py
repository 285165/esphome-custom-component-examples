...
ENCODER_SCHEMA = sensor.sensor_schema(accuracy_decimals=0, icon="mdi:knob").extend({
    cv.Required(CONF_NUMBER): cv.int_range(min=0, max=7),
})
BUTTON_SCHEMA = binary_sensor.binary_sensor_schema(icon="mdi:gesture-tap-button").extend({
    cv.Required(CONF_NUMBER): cv.int_range(min=0, max=7),
