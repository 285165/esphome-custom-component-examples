...
bool M5Stack8Encoder::reset_all_counters() {
  ...
  // Registers 0x40..0x47 are eight independent write-only command registers.
  // One Home Assistant button press triggers eight separate I2C writes.
  for (uint8_t channel = 0; channel < 8; channel++) {
    const uint8_t reg = REG_RESET_BASE + channel;
    if (!this->write_byte(reg, 1)) {
      ESP_LOGE(TAG, "Failed to reset CH%u using register 0x%02X",
               channel + 1, reg);
      all_ok = false;
    }
  }
...
void M5Stack8Encoder::update() {
  ...
  // Registers 0x20..0x3F are read-only increment/delta values,
  // not configuration registers.
  for (uint8_t channel = 0; channel < 8; channel++) {
    if (this->increment_sensors_[channel] == nullptr)
      continue;

    uint8_t data[4];
    const uint8_t reg = REG_INCREMENT_BASE + channel * 4;
    if (!this->read_regs_(reg, data, sizeof(data))) {
      ESP_LOGW(TAG, "Failed to read CH%u increment", channel + 1);
      all_ok = false;
      continue;
    }
    const uint32_t raw = static_cast<uint32_t>(data[0]) |
                         ...
                         (static_cast<uint32_t>(data[2]) << 16) |
                         (static_cast<uint32_t>(data[3]) << 24);
    this->increment_sensors_[channel]->publish_state(
        static_cast<float>(static_cast<int32_t>(raw)));
  }
